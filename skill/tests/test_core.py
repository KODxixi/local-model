...
from knowledge_graph import Entity, KnowledgeGraph, extract_entities_regex
...
def test_extract_entities_regex():
    """规则提取实体。"""
    text = '这个项目叫"智慧社区"，使用了《建筑设计规范》。'
    entities = extract_entities_regex(text)
    assert len(entities) >= 1
    names = [e.name for e in entities]
    assert "智慧社区" in names or "建筑设计规范" in names


def test_knowledge_graph_add_and_find():
    """知识图谱：添加实体，然后查找。"""
    with tempfile.TemporaryDirectory() as tmpdir:
        kg = KnowledgeGraph(db_path=tmpdir, kb_name="test_kg")

        # 添加实体
        entities = [
            Entity(name="Python", entity_type="tech", description="编程语言"),
            Entity(name="LanceDB", entity_type="tech", description="向量数据库"),
        ]
        content = "Python 和 LanceDB 一起使用效果很好。Python 是常用语言。"
        count = kg.add_entities(entities, "/test/doc.md", content)
        assert count == 2

        # 查找提到 Python 的文档
        docs = kg.find_docs_by_entity("Python")
        assert len(docs) >= 1
        assert docs[0]["path"] == "/test/doc.md"
...
def test_knowledge_graph_related():
    """知识图谱：查找共现实体。"""
    with tempfile.TemporaryDirectory() as tmpdir:
        kg = KnowledgeGraph(db_path=tmpdir, kb_name="test_kg2")

        # 两个文档都提到 Python 和 LanceDB
        entities1 = [
            Entity(name="Python", entity_type="tech"),
            Entity(name="LanceDB", entity_type="tech"),
        ]
        kg.add_entities(entities1, "/doc1.md", "Python with LanceDB")

        entities2 = [
            Entity(name="Python", entity_type="tech"),
            Entity(name="SQLite", entity_type="tech"),
        ]
        kg.add_entities(entities2, "/doc2.md", "Python with SQLite")

        # 找与 Python 共现的实体
        related = kg.find_related_entities("Python")
...
def test_knowledge_graph_delete():
    """知识图谱：按路径删除。"""
    with tempfile.TemporaryDirectory() as tmpdir:
        kg = KnowledgeGraph(db_path=tmpdir, kb_name="test_kg3")
        entities = [Entity(name="Test", entity_type="concept")]
        kg.add_entities(entities, "/tmp.md", "Test content")
        assert kg.stats()["unique_entities"] == 1

        deleted = kg.delete_by_path("/tmp.md")
        assert deleted >= 1
        assert kg.stats()["unique_entities"] == 0
...
def run_all():
    tests = [
        ...
        test_chunk_code_block_protected,
        test_chunk_table_protected,
        test_chunk_small_merge,
        test_chunk_overlap,
        test_chunk_document_dispatch,
        test_chunk_empty,
        # ingest
        test_guess_doc_type,
        test_ingest_text_file,
        test_ingest_nonexistent,
        test_safe_read_text_encoding,
        # vector_store
        test_make_schema,
        test_vector_store_create_and_stats,
        test_vector_store_search,
        test_vector_store_incremental_update,
        test_vector_store_delete_by_path,
        # knowledge_graph
        test_extract_entities_regex,
        test_knowledge_graph_add_and_find,
        test_knowledge_graph_related,
